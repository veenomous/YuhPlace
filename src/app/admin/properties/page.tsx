...
interface AdminPropertyListing {
  id: string;
  title: string;
  ...
  price_amount: number;
  currency: string;
  status: 'active' | 'rented' | 'sold' | 'hidden' | 'removed';
  is_featured: boolean;
  created_at: string;
  owner_name: string;
  owner_id: string;
...
export default function AdminPropertiesPage() {
  const [listings, setListings] = useState<AdminPropertyListing[]>([]);
  ...
  const fetchListings = useCallback(async () => {
    ...
    const { data } = await supabase.rpc('get_admin_property_listings');
    if (data) setListings(data as AdminPropertyListing[]);
    setLoading(false);
  }, []);
  ...
  const filteredListings = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      results = results.filter(
        (l) => l.title.toLowerCase().includes(q) || l.owner_name.toLowerCase().includes(q),
      );
    }
    ...
    if (modeFilter !== 'all') results = results.filter((l) => l.listing_mode === modeFilter);
    return results;
  }, [listings, search, statusFilter, modeFilter]);
  ...
  async function handleAction(id: string, action: 'hide' | 'remove' | 'restore') {
    ...
    if (action === 'hide') {
      ({ error } = await supabase.rpc('admin_hide_content', { p_target_type: 'property_listing', p_target_id: id }));
    } else if (action === 'remove') {
      ({ error } = await supabase.rpc('admin_remove_content', { p_target_type: 'property_listing', p_target_id: id }));
    } else {
      ({ error } = await supabase.rpc('admin_restore_content', { p_target_type: 'property_listing', p_target_id: id }));
    }

    if (!error) {
      const newStatus = action === 'hide' ? 'hidden' : action === 'remove' ? 'removed' : 'active';
      setListings((prev) => prev.map((l) => (l.id === id ? { ...l, status: newStatus as AdminPropertyListing['status'] } : l)));
    }
  ...
  async function handleToggleFeatured(id: string) {
    setActionLoading(id);
    const supabase = createClient();
    const { data: newVal, error } = await supabase.rpc('admin_toggle_featured', { p_target_type: 'property_listing', p_target_id: id });
    if (!error) {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted mt-1">Manage rental and sale property listings</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <div className="relative">
          <select value={modeFilter} onChange={(e) => setModeFilter(e.target.value)} className="appearance-none bg-white border border-border rounded-lg pl-3 pr-8 py-2 text-sm text-foreground cursor-pointer hover:border-primary/30 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
            {MODE_FILTER_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
          <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted pointer-events-none" />
        </div>
        {(statusFilter !== 'all' || modeFilter !== 'all' || search.trim()) && (
          <button onClick={() => { setStatusFilter('all'); setModeFilter('all'); setSearch(''); }} className="text-sm text-primary hover:underline font-medium">Clear filters</button>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-surface/50">
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted uppercase tracking-wide">Property</th>
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted uppercase tracking-wide">Owner</th>
            ...
            <tbody>
              {filteredListings.length === 0 ? (
                <tr><td colSpan={6} className="px-5 py-12 text-center text-muted">{listings.length === 0 ? 'No property listings yet.' : 'No listings match the selected filters.'}</td></tr>
              ) : (
                    ...
                    <tr key={listing.id} className={cn('border-b border-border last:border-b-0 transition-colors', listing.status !== 'active' ? 'bg-surface/20' : 'hover:bg-surface/30')}>
                      <td className="px-5 py-3 align-top">
                        <div>
                          <div className="flex items-center gap-1.5">
                            <Link href={`/property/${listing.id}`} target="_blank" className="font-medium text-foreground hover:text-primary transition-colors line-clamp-1">{listing.title}</Link>
                      ...
                      <td className="px-5 py-3 align-top">
                        <div className="text-xs">
                          <span className={cn('inline-flex px-2 py-0.5 rounded-full font-medium', listing.listing_mode === 'rent' ? 'bg-blue-50 text-blue-700' : 'bg-green-50 text-green-700')}>
                            {listing.listing_mode === 'rent' ? 'Rent' : 'Sale'}
                          </span>
                          <p className="text-muted mt-1">{PROPERTY_TYPE_LABELS[listing.property_type] || listing.property_type}</p>
                        </div>
                      </td>
                      <td className="px-5 py-3 text-foreground font-medium align-top whitespace-nowrap">
                        {formatPrice(listing.price_amount, listing.currency)}
                        {listing.listing_mode === 'rent' && <span className="text-muted font-normal">/mo</span>}
                      </td>
                      <td className="px-5 py-3 text-foreground align-top">{listing.owner_name}</td>
                      <td className="px-5 py-3 align-top">
                      ...
                      <td className="px-5 py-3 align-top">
                        <div className="flex items-center justify-end gap-1.5">
                          {isProcessing ? (
                            ...
                            <>
                              <Link href={`/property/${listing.id}`} target="_blank" className="p-1.5 text-muted hover:text-primary hover:bg-primary-light rounded-lg transition-colors" title="View property"><Eye size={16} /></Link>
